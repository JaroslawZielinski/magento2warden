...
do
    for k in `mysql --defaults-extra-file=$cfg $db -e "DESC $i" | grep -v \`mysql --defaults-extra-file=$cfg $db -e "DESC $i" | head -1\` | grep -v int | awk '{print $1}'`
    do
        if [ `mysql --defaults-extra-file=$cfg $db -e "SELECT * FROM $i WHERE LOWER($k) LIKE '%$w%'" | wc -l` -gt 1 ]
        then
            mysql --defaults-extra-file=$cfg $db -e "SELECT * FROM $i WHERE LOWER($k) LIKE '%$w%';" >> "${i}-${k}.txt"
	    count=$((count+1))
        fi
    done
done
